...
for filename in iglob(input_dir + '/*.png'):
    ...
    if mode == 'base':
        for orientation in ('horizontal', 'vertical'):
            a = load(file)
            equiv = a == seam_coords(seam_carve(img, orientation + ' shrink')[2])
            print(equiv)
            TP += equiv
        ...
        for m in (None, msk):
            for direction in ('shrink', 'expand'):
                for orientation in ('horizontal', 'vertical'):
                    equiv = load(file) == seam_coords(seam_carve(img, orientation + ' ' + direction, mask = m)[2])
                    TP += equiv
                    print(m, direction, orientation, equiv)
